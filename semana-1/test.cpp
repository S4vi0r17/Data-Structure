...
int Mezclar(FILE *fich, FILE **aux)
{
    ...
    strcpy(anterior[0], "");
    strcpy(anterior[1], "");
    // Bucle, mientras no se acabe ninguno de los archivos auxiliares(quedan tramos por mezclar) :
    while (!feof(aux[0]) && !feof(aux[1]))
    {
        ...
        // Verifica fin de tramo, si termino copia el resto del tramo:
        if (strcmp(anterior[entrada], linea[entrada]) > 0)
        {
            if (!entrada)
                ...
                entrada = 0;
            tramos++;
            // Copia lo que queda del tramo actual al archivo de Salida;
            do
            {
                strcpy(anterior[entrada], linea[entrada]);
                fputs(linea[entrada], fich);
